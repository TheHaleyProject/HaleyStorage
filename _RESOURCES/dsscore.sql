...
CREATE TABLE IF NOT EXISTS `workspace` (
  ...
  `storage_ref` varchar(200) NOT NULL COMMENT 'Relative path (FS) or object-key prefix (cloud) for this workspace root, computed from the workspace name hash via sharding at registration time.\nFor FS example: "f1/2e/3b/" — a shard directory under the module base path on disk.\nFor cloud example: "f12e3b/" — a flat prefix prepended to all object keys in this workspace.',
  `active` bit(1) NOT NULL DEFAULT b'1' COMMENT '1 = accepts uploads. 0 = soft-disabled; reads still work.',
  `storagename_mode` int(11) NOT NULL DEFAULT 1 COMMENT 'Controls how the physical storage file name is chosen for each file uploaded into this workspace:\n1 - Number mode: the auto-increment database id is used (e.g. doc_version.id = 1234 → file saved as "1234f.pdf"). Compact and fast to look up.\n2 - Guid mode: a compact-N UUID is used (e.g. "a3b2c1d4e5f67890...f.pdf"). Globally unique without a DB round-trip; preferred for distributed or multi-node deployments.',
