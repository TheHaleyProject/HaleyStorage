...
namespace Haley.Services {
    public partial class StorageCoordinator : IStorageCoordinator {
        ...
        public StorageCoordinator(string basePath, bool write_mode, IStorageIndexing indexer, bool throwExceptions, ILogger logger =null) {
        ...
        public static IStorageCoordinator Create(IAdapterGateway agw, string adapter_key, out (string logpath,string respMode) data, bool throwExceptions = false) {
            ...
            var dirPath = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? StorageConstants.CONFIG_DIR_WIN : StorageConstants.CONFIG_DIR_LINUX;
            var dirSection = cfgRoot[$@"Seed:{StorageConstants.CONFIG_INFO}:{dirPath}"];
            var responseMode = cfgRoot[$@"Seed:{StorageConstants.CONFIG_INFO}:{StorageConstants.CONFIG_DIR_RESPONSEPATHMODE}"];
            bool writemode = false;
            bool.TryParse(ResourceUtils.FetchVariable($@"{StorageConstants.OSS_WRITEMODE}")?.Result?.ToString(), out writemode);
            ...
            var allowedFormats = cfgRoot[$@"Seed:{StorageConstants.OSS_FILEFORMATS}:{StorageConstants.ALLOWED}"];
            if (!string.IsNullOrWhiteSpace(allowedFormats)) dss.AddFormatRange(allowedFormats.Split(',')?.ToList(), FormatControlMode.Extension);
            var allowedMimes = cfgRoot[$@"Seed:{StorageConstants.OSS_FILEFORMATS}:{StorageConstants.ALLOWED_MIMETYPE}"];
            if (!string.IsNullOrWhiteSpace(allowedMimes)) dss.AddFormatRange(allowedMimes.Split(',')?.ToList(), FormatControlMode.MimeType);

            var restrictedFormats = cfgRoot[$@"Seed:{StorageConstants.OSS_FILEFORMATS}:{StorageConstants.RESTRICTED}"];
            ...
            var restrictedMimes = cfgRoot[$@"Seed:{StorageConstants.OSS_FILEFORMATS}:{StorageConstants.RESTRICTED_MIMETYPE}"];
            ...
            var throwEx = cfgRoot[$@"Seed:{StorageConstants.THROW_EX}"];
        ...
        IStorageIndexing Indexer;

        public StorageCoordinator SetWriteMode(bool mode) {
            WriteMode = mode;
            return this;
        }
        public IStorageCoordinator SetIndexer(IStorageIndexing service) {
            Indexer = service;
